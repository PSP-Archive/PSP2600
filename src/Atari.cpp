...
myCtrlPeekBufferPositive( SceCtrlData* pc, int count )
{
  ...
  /* too fast ? */
  if ((pc->TimeStamp - last_time) < 1600) {
...
atari_treat_command_key(int atari_idx)
{
  ...
  switch (atari_idx) 
  {
    ...
    break;
    case ATARIC_DECFLICK:
      if (ATARI.atari_flicker_mode > 0) ATARI.atari_flicker_mode--;
    break;
    case ATARIC_INCFLICK: 
      if (ATARI.atari_flicker_mode <  ATARI_LAST_FLICKER) ATARI.atari_flicker_mode++;
    break;

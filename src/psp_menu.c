...
psp_display_screen_menu(void)
{
  ...
  for (slot_id = 0; slot_id < ATARI_MAX_SAVE_STATE; slot_id++) {
    ...
    if (ATARI.atari_save_state[slot_id].used) {
      ScePspDateTime* my_date = &ATARI.atari_save_state[slot_id].date;
# ifndef LINUX_MODE
...
# endif
  ...
  if (ATARI.atari_save_state[cur_slot].thumb) {
    psp_sdl_blit_thumb(320, 40, ATARI.atari_save_state[cur_slot].surface);
  } else {
...
psp_main_menu_reset(void)
{
  ...
  psp_sdl_back2_print(140, 160, "Reset Atari !", PSP_MENU_WARNING_COLOR);
  psp_menu_dirty = 1;
  psp_sdl_flip();
